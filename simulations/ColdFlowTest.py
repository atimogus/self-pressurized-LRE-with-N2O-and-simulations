...
class ColdFlowTestSimulation:
    ...
    def run_liquid_phase_simulation(self, iteration, pressure_old, o_f_old):
        ...
        if self.mass_liquid_ox[iteration] < self.config.OXIDIZER_MASS_KG * 0.5 and (
            pressure_old - ox_pressure_liquid < np.mean(self.dp_dt[-5:]) * 0.5):
            
            finetune_value = 5 #smaller numebr is better because it is less of constraint on the simulation
        ...
        ox_liquid_massflowrate = self.injector.calculate_mass_flow_rate_SPI(ox_density_liquid, ox_pressure_liquid, self.config.P_ATMOSPHERE) * self.config.TIME_STEP
        ...
        if self.mass_fuel[iteration] > 0:
            ...
            massflowrate_fuel = self.injector.calculate_fuel_massflowrate_SPI( density_fuel, pressure_fuel, self.config.P_ATMOSPHERE ) * self.config.TIME_STEP
            ...
            #povecanje volumena koj plin mora zauzeti
            self.tank.volume_tank_oxidizer += (massflowrate_fuel / density_fuel)
    ...
    def run_vapor_phase_simulation(self, iteration):
        ...
        ox_vapour_massflowrate = self.injector.calculate_mass_flow_rate_SPI( 
            density_vapour, self.ox_pressure_liquid[iteration], self.config.P_ATMOSPHERE) * self.config.TIME_STEP
    ...
    def plot_simulation_results(self):
        ...
        with open("simulations/coldflowtest-pressure.json", "r") as f:
            exp_data = json.load(f)
        exp_time = np.array(exp_data["x"])
        exp_pressure = np.array(exp_data["y"]) * 1e5
