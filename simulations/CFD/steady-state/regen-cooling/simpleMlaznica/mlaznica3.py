...
print(f"brzina_protoka_IPA: {brzina_protoka_IPA:.2f} m/s\n")
k = 1.                                                                         #koeficijent koliko je izlazna povrsina veca od ulazne k=1 ,jednake su, k=2 izlazna je dvostruko veca od ulazne
Auk = ((d_hidraulicke_cijevi/1000)**2 /4)*np.pi                                #m^2
n = 12
          
#debljina stjenke za rashladne kanale                                                           # broj kanala zasebnih
stjenka_alu = 2 #mm
...
print(stjenka_alu)
print(unutarnja_stjenka_rashladnih_kanala)
fi_alu_rad = 2 * (stjenka_alu) / (unutarnja_stjenka_rashladnih_kanala)
...
stjenka_alu_ulaz_mlaznice = 7 #mm
# fi_alu_rad_ulaz_mlaznice = 2 * stjenka_alu_ulaz_mlaznice / D_ulaz_mlaznice
...
print(f"fi_IPA_ulaz_deg: {fi_IPA_ulaz_gore_deg:.6f}°")
print(f"duzina_IPA_ulaz_kanala: {duzina_IPA_ulaz_gore_kanala*1000:.4f} mm")
...
print(f"fi_IPA_izlaz_deg: {fi_IPA_izlaz_gore_deg:.6f}°")
print(f"duzina_IPA_izlaz_kanala: {duzina_IPA_izlaz_gore_kanala*1000:.4f} mm\n")
...
visina_prolaz_izmedju_kanala = 3 * (povrsina_cijevi / (n*(d_vani_grlo - unutarnja_stjenka_rashladnih_kanala/1000)))
...
povrsina_kanala_gore = ((D_ulaz_mlaznice/1000)**2 - (d_ulaz_mlaznice)**2) * fi_IPA_ulaz_gore_rad/8

povrsina_kanala_grlo = ((d_vani_grlo)**2 - (unutarnja_stjenka_rashladnih_kanala/1000)**2) * fi_IPA_ulaz_rad/8

print(f"povrsina_kanala_gore = {povrsina_kanala_gore}m^2")
print(f"povrsina_kanala_grlo = {povrsina_kanala_grlo}m^2")

brzina_gore = (maseni_protok_IPA / 12) / (gustoca_IPA * povrsina_kanala_gore)
brzina_kanala_grlo = (maseni_protok_IPA / 12) / (gustoca_IPA * povrsina_kanala_grlo)
print(f"brzina_gore: {brzina_gore:.2f} m/s")
print(f"brzina_kanala_grlo: {brzina_kanala_grlo:.2f} m/s\n")
...
def export_facebinders_to_stl():
    output_dir = os.path.join(os.path.dirname(doc.FileName), "cases", "geometrija")
