...
def generate_nozzle_geometry(y_combustion_chamber, y_exit, y_throat, num_of_points_nozzle):
    ...
    y_offset = yend * 2  # Offset by twice the nozzle exit height
...
def plot_nozzle_contour(points_list):
    """Plot the nozzle contour including combustion chamber and downstream domain"""
    print("=== STEP 2: Plotting Complete Geometry Contour ===")
    ...
    plt.plot(x_coords, y_coords, 'b-o', linewidth=2, markersize=3, label='Upper Wall')
    plt.plot(x_coords, [-y for y in y_coords], 'r-o', linewidth=2, markersize=3, label='Lower Wall')
    
    # Add centerline
    plt.axhline(y=0, color='k', linestyle='--', alpha=0.5, label='Centerline')
    
    # Add vertical lines to separate domains
    chamber_nozzle_boundary = 0.0
    nozzle_downstream_boundary = None
    
    # Find nozzle-downstream boundary
    for i, x in enumerate(x_coords):
        if i > 0 and y_coords[i] == y_coords[i-1] and x > chamber_nozzle_boundary + 1:
            nozzle_downstream_boundary = x_coords[i-1]
            break
    
    if nozzle_downstream_boundary:
        plt.axvline(x=chamber_nozzle_boundary, color='g', linestyle=':', alpha=0.7, label='Chamber-Nozzle')
        plt.axvline(x=nozzle_downstream_boundary, color='m', linestyle=':', alpha=0.7, label='Nozzle-Downstream')
    
    # Formatting
    plt.xlabel('X Position', fontsize=12)
    plt.ylabel('Y Position', fontsize=12)
    plt.title('Complete Nozzle System: Combustion Chamber + Nozzle + Downstream Domain', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.axis('equal')
    
    # Add annotations
    throat_idx = np.argmin(y_coords)
    chamber_start_idx = 0
    downstream_end_idx = len(x_coords) - 1
    
    plt.annotate('Chamber Inlet', xy=(x_coords[chamber_start_idx], y_coords[chamber_start_idx]), 
                xytext=(x_coords[chamber_start_idx]-0.5, y_coords[chamber_start_idx]+0.5),
                arrowprops=dict(arrowstyle='->', color='black'), fontsize=10)
    plt.annotate('Throat', xy=(x_coords[throat_idx], y_coords[throat_idx]), 
                xytext=(x_coords[throat_idx], y_coords[throat_idx]-1.0),
                arrowprops=dict(arrowstyle='->', color='black'), fontsize=10)
    plt.annotate('Downstream Exit', xy=(x_coords[downstream_end_idx], y_coords[downstream_end_idx]), 
                xytext=(x_coords[downstream_end_idx]+0.3, y_coords[downstream_end_idx]+0.5),
                arrowprops=dict(arrowstyle='->', color='black'), fontsize=10)
    
    plt.tight_layout()
    plt.show()
    
    print("Complete geometry contour plot displayed")
    print(f"Geometry validation:")
    print(f"  - Chamber area ratio: 1.0")
    print(f"  - Throat area ratio: {abs(y_coords[throat_idx])/y_coords[0]:.3f}")
    print(f"  - Exit area ratio: {y_coords[-1]/y_coords[0]:.3f}")
    print(f"  - Total length: {x_coords[-1] - x_coords[0]:.3f}\n")
